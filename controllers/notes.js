...
notesRouter.get('/:id', (req, res, next) => {
  Note.findById(req.params.id)
    .then(note => (note ? res.json(note.toJSON()) : res.status(404).send({ error: 'note not found on DB' })))
    .catch(e => next(e))
})
...
notesRouter.delete('/:id', (req, res, next) => {
  Note.findByIdAndRemove(req.params.id)
    .then(delNote => (delNote ? res.status(204).end() : res.status(404).send({ error: 'note not found' })))
